...
import base_model
from base_model import IOG, Discriminator
...
def main():
    ...
    if dataset=='cpv1':
        dictionary = Dictionary.load_from_file('tools/data/dictionary_v1.pkl')
    elif dataset=='cpv2' or dataset=='v2':
        dictionary = Dictionary.load_from_file('tools/data/dictionary.pkl')
    ...
    iog = IOG(num_hid=1024, dataset=train_dset).cuda()
    discriminator = Discriminator(num_hid=1024, dataset=train_dset).cuda()
    if dataset=='cpv1':
        model.w_emb.init_embedding('tools/data/glove6b_init_300d_v1.npy')
        iog.w_emb.init_embedding('tools/data/glove6b_init_300d_v1.npy')
    elif dataset=='cpv2' or dataset=='v2':
        model.w_emb.init_embedding('tools/data/glove6b_init_300d.npy')
        iog.w_emb.init_embedding('tools/data/glove6b_init_300d.npy')
    ...
    train(model, iog, discriminator, train_loader, eval_loader, args,qid2type)
