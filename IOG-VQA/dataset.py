...
def _load_dataset(dataroot, name, img_id2val, dataset):
    ...
    """
    if dataset=='cpv2':
      answer_path = os.path.join('tools',dataroot, 'cp-cache', '%s_target.pkl' % name)
      name = "train" if name == "train" else "test"
      question_path = os.path.join('tools',dataroot, 'vqacp_v2_%s_questions.json' % name)
      with open(question_path) as f:
        questions = json.load(f)
    elif dataset=='cpv1':
      answer_path = os.path.join('tools',dataroot, 'cp-v1-cache', '%s_target.pkl' % name)
      name = "train" if name == "train" else "test"
      question_path = os.path.join('tools',dataroot, 'vqacp_v1_%s_questions.json' % name)
      with open(question_path) as f:
        questions = json.load(f)
    elif dataset=='v2':
      answer_path = os.path.join('tools',dataroot, 'cache', '%s_target.pkl' % name)
      question_path = os.path.join('tools',dataroot, 'v2_OpenEnded_mscoco_%s2014_questions.json' % name)
      with open(question_path) as f:
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', dataset='cpv2',
        ...
        if dataset=='cpv2':
            ans2label_path = os.path.join('tools', dataroot, 'cp-cache', 'trainval_ans2label.pkl')
            label2ans_path = os.path.join('tools', dataroot, 'cp-cache', 'trainval_label2ans.pkl')
        elif dataset=='cpv1':
            ans2label_path = os.path.join('tools', dataroot, 'cp-v1-cache', 'trainval_ans2label.pkl')
            label2ans_path = os.path.join('tools', dataroot, 'cp-v1-cache', 'trainval_label2ans.pkl')
        elif dataset=='v2':
            ans2label_path = os.path.join('tools', dataroot, 'cache', 'trainval_ans2label.pkl')
            label2ans_path = os.path.join('tools', dataroot, 'cache', 'trainval_label2ans.pkl')
        ...
        if use_hdf5:
            ...
            self.spatial = {}
            self.image_id2ix = {}
            h5_path_train = os.path.join('tools',dataroot, 'detection_features','train36.hdf5')
            ...
            h5_path_test = os.path.join('tools',dataroot, 'detection_features','val36.hdf5')
            ...
            self.image_id2ix['train'] = pickle.load(open(os.path.join('tools',dataroot, 'detection_features', f'train36_imgid2img.pkl'), 'rb'))
            self.image_id2ix['val'] = pickle.load(open(os.path.join('tools',dataroot, 'detection_features', f'val36_imgid2img.pkl'), 'rb'))
        else:
