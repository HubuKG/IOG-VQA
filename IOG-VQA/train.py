...
def calc_iog_loss(logits, bias, labels):
...
def train(model, iog, discriminator, train_loader, eval_loader,args,qid2type):
    num_epochs=args.epochs
    run_eval=args.eval_each_epoch
    output=args.output
    optim = torch.optim.Adamax(filter(lambda p: p.requires_grad, model.parameters()), lr=0.001)
    optim_G = torch.optim.Adamax(filter(lambda p: p.requires_grad, iog.parameters()), lr=0.001)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, qid) in tqdm(enumerate(train_loader), ncols=100, desc="Epoch %d" % (epoch + 1), total=len(train_loader)):
            ...
            g_loss += dsc_loss + g_distill*4
            g_loss_clone = g_loss.clone()
            g_loss_clone.backward(retain_graph=True)
            nn.utils.clip_grad_norm_(iog.parameters(), 0.50)
            # done training iog
            
            # train the discriminator
            dsc_loss = bce(vae_preds, fake) + bce(main_preds, valid)
            dsc_loss_clone = dsc_loss.clone()
            dsc_loss_clone.backward(retain_graph=True)
            optim_G.step()
            optim_D.step()
            # done training the discriminator

            
            # use iog to train the robust model
            iog.train(False)
            with torch.no_grad():
                pred_g = iog(v, q, gen=False)

            iog_loss = calc_iog_loss(pred, pred_g, a)
            iog_loss_clone = iog_loss.clone()
            iog_loss_clone.backward()

            nn.utils.clip_grad_norm_(model.parameters(), 0.50)
            optim.step()

            iog.train(True)
            total_loss += iog_loss.item() * q.size(0)
        ...
        if run_eval:
            ...
            if main_eval_score > best_eval_score:
                ...
                iog_path = os.path.join(output, 'iog.pth')
                torch.save(model.state_dict(), model_path)
                torch.save(iog.state_dict(), iog_path)
